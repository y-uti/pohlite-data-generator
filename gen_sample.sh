...
# 会社数 (n)
for n in 50 100 200 300 500 1000 2000 3000 5000 10000 20000 30000 50000; do
    ...
    # 人員数   (qi) : 形状母数 2, 尺度母数 500 のガンマ分布
    # 費用     (ri) : 平均 100, 標準偏差 10 の正規分布 * 人員数
    # 必要人数 (m)  : n * 平均人員数 / 2
    datafile=$DATADIR/data_gn_$(printf "%05d" $n).txt
    php $PROG -n $n -q g:2:500 -r n:100:10 >$datafile

    # 人員数   (qi) : 平均 10 のポアソン分布
    # 費用     (ri) : 平均 100, 標準偏差 10 の正規分布 * 人員数
    # 必要人数 (m)  : n * 平均人員数 / 2
    datafile=$DATADIR/data_pn_$(printf "%05d" $n).txt
    php $PROG -n $n -q p:10 -r n:100:10 >$datafile
